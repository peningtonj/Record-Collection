...
/**
 * Pure-Kotlin SHA-256 — the browser only offers the *async* `crypto.subtle.digest`, but
 * [sha256Hex] must stay synchronous (`generateAlbumId` is called from mappers). Small and
 * only used for stable album ids; `GenerateAlbumIdTest` locks the digest against the JVM
 * implementation.
 */
...
actual fun sha256Hex(input: String): String {
    ...
    for (v in h) {
        for (i in 7 downTo 0) {
            sb.append("0123456789abcdef"[(v ushr (i * 4)) and 0xf])
        }
    }
    return sb.toString()
}
